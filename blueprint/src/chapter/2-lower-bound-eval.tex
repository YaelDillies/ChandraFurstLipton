...
\begin{theorem}[Lower bound for $D(\eval)$ in terms of $\chi_d(G)$]
  ...
  \uses{def:det-complex, def:eval, def:corner-color-num}
  % \lean{}
  % \leanok
...
\begin{corollary}[Upper bound for $D(\eval)$ in terms of $r_d(G)$]
  ...
  \uses{def:det-complex, def:eval, def:corner-free-num}
  % \lean{}
  % \leanok
...
\end{corollary}
\begin{proof}
  \uses{thm:det-complex-eval-ge-corner-color-num, lem:corner-num-lower}
  % \leanok
